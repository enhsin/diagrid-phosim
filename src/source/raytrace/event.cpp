...
// *************************************************************
{
    ...
    pfPhotonLog->pX=new float[pfPhotonLog->bufferSize];
    pfPhotonLog->pY=new float[pfPhotonLog->bufferSize];
    pfPhotonLog->pZ=new float[pfPhotonLog->bufferSize];
...
// *************************************************************************
{
    ...
    pfPhotonLog->pX[logIndex]       = (float)a;
    pfPhotonLog->pY[logIndex]       = (float)b;
    pfPhotonLog->pZ[logIndex]       = (float)c;
...
// ********************************************************************
{
    ...
    // std::cout<<"Event File at: "<<tempstring<<std::endl;
    ...
    if(status!=0){
        std::cout<<"eventFileCreate failed to create event file: "<<tempstring
    ...
    status=0; //Must always init this to 0=ok before calling cfitsio routine*
    fits_create_tbl(pfPhotonLog->pFitsFile,BINARY_TBL,0,4,ttype,tform,NULL,NULL,
                    &status);
    if(status!=0){
        std::cout<<"eventFileCreate failed to create event file table. Status:"
                 <<status<<std::endl;
        pfPhotonLog->pFitsFile=NULL;
        return;
    }
...
// **********************************************************************
{
    int status;
    status=0; //Must always init this to 0=ok before calling cfitsio routine*
    fits_write_col(pfPhotonLog->pFitsFile,TFLOAT,1,firstRow,1,numRowsToWrite,
                   pfPhotonLog->pX,&status);
    fits_write_col(pfPhotonLog->pFitsFile,TFLOAT,2,firstRow,1,numRowsToWrite,
                   pfPhotonLog->pY,&status);
    fits_write_col(pfPhotonLog->pFitsFile,TFLOAT,3,firstRow,1,numRowsToWrite,
                   pfPhotonLog->pZ,&status);
    fits_write_col(pfPhotonLog->pFitsFile,TINT,4,firstRow,1,numRowsToWrite,
                   pfPhotonLog->pSurface,&status);
    if(status!=0){
        std::cout<<"eventFileWrite failed to write data to event file. Status: "
                 <<std::endl;
...
// ************************************************************************
// Clean up (write partial bufffers if we need to) and close up the events*
// fits file
// ************************************************************************
{
    int status;
    ...
    if(pfPhotonLog->pFitsFile!=NULL){
        ...
        if(status!=0){
            std::cout<<"eventFileClose failed to close event file. Status: "
                     <<std::endl;
        }
    }
