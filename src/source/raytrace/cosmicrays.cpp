...
int *transform_array(int* matrix, int cols, int rows, int ro, int f);
        ...
        for (l=0;l<pixelsy;l++) {
            ...
            if(nm<probability)    {
                ...
                if(rnd!=1.0)
                    ...
                    filenum=130;
                char str[4];
                char rayfile[80]; /*indicate directory/location of ray files*/
                ...
                *raynumber=*raynumber+1;
                ...
                int *newmatrix=NULL;
                int value;
                newmatrix=(int *) malloc(sizeof(int)*cols*rows);
                newmatrix=transform_array(*data, cols, rows,ro,fl );
                ...
                for(k=0;k<rows2;k++) {
                    for(j=0;j<cols2;j++) {    
                        ...
                        if (xPos >= minx && xPos <= maxx &&    yPos >= miny && yPos <= maxy) {
                            ...
                            if (value<0) value=0;
                            *(chip.focal_plane+chip.nampx*(yPos-miny)+(xPos-minx))+=scalenumber*value;
                            if (*(chip.focal_plane+chip.nampx*(yPos-miny)+(xPos-minx))>well_depth) 
                                *(chip.focal_plane+chip.nampx*(yPos-miny)+(xPos-minx))=well_depth;
                        }
...
int *transform_array(int* matrix, int cols, int rows, int ro, int fl)
{
    int *newmatrix=NULL;
    int *tempmatrix=NULL;
    tempmatrix=(int *) malloc(sizeof(int)*cols*rows);
    newmatrix=(int *) malloc(sizeof(int)*cols*rows);
    int x=0,y=0,i=0;
    //flip vertically
    if(fl==0) {
