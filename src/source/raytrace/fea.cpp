...
void Fea::knnQuery(std::vector<double> & x, std::vector<double> & y, std::vector<double> & z, size_t nNear, double p) {
    for (size_t t(0);t<x.size();t++) {
        ...
        for  (size_t n(0);n<nNear;n++) {
            ...
            double w=pow(out_dists_sqr[n],-p/2.);
...
void Fea::knnQueryFitDegree1(std::vector<double> & x, std::vector<double> & y, double *z, double *z_r, double *z_phi, size_t nNear) {
    ...
    for (size_t t(0);t<x.size();t++) {
        ...
        if (sameNeighbors==0) {
            ...
            /* ct = transpose(A) b */
            double s0(0.),sx(0.), sy(0.), vx(0.), vy(0.), xy(0.), a1, a2;
            double ct[3]={0,0,0};
            for  (size_t n(0);n<nNear;n++) {
                size_t idx=ret_indexes[n];
                if (coordType==1) {
            ...
            for (size_t i(0); i<3; i++) c0[i]=c[i];
            t0=t;
        } else {
            for (size_t i(0); i<3; i++) c[i]=c0[i];
        }
        z[t]=c[0];
        z_r[t]=0.;
        z_phi[t]=0.;
...
void Fea::subtractSurface(Surface & surface, long surfaceIndex) {
    rmax=surface.rmax[surfaceIndex];
    ...
    if (radiusofcurv !=0) {
        ...
        for (size_t t(0);t<m_z.size();t++) m_z[t]-=surface.height[surfaceIndex];
    }
}
