...
pro validation_2C,nnn,vers,value,tolerance_low,tolerance_high,task,name,unit,comparison
  ...
  for ii=0L,17 do begin
     ...
     if ii le 5 then begin
        tname='throughput_lsst_e_220'+string(filter,format='(I1)')+'_R22_S11_E00'
        cc=250 & lc=1
     endif
     if ii ge 6 and ii le 11 then begin
        tname='throughput_lsst_e_221'+string(filter,format='(I1)')+'_R22_S11_E00'
        cc=50 & lc=1
     endif
     if ii ge 12 and ii le 17 then begin
        tname='throughput_lsst_e_222'+string(filter,format='(I1)')+'_R24_S12_E00'
        cc=130 & lc=1
     endif
     tname1=tname+'0.txt'
     tname2=tname+'1.txt'
